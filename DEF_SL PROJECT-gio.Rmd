...
FIFA is one of the most known videogame and the most famous sport title in the industry, in particular we considered FIFA 22 edition. Each player covers a specific position on the field; what we want to do in the first part of the project is building some models to classify the position of the player, based on the values of its attributes. It's important to consider that some players may share some features with footballers playing in another position, and this may influence our task. For example, some attacking midfielders (CAM) have a good shot and pace, just like wingers (RW, LW). We will keep this into account and adjust our classification accordingly. In the second part of our project we will perform a regression task, building some models to evaluate the market price of a football player, using Ridge and Lasso regression.
...
players_22 <- subset(players_full, select = c("short_name","player_positions","age", "value_eur", "height_cm","weight_kg","pace","shooting","passing","preferred_foot","weak_foot","dribbling","defending","physic","attacking_crossing","attacking_finishing","attacking_heading_accuracy","attacking_short_passing","attacking_volleys","skill_dribbling","skill_curve","skill_fk_accuracy","skill_long_passing","skill_ball_control","movement_acceleration","movement_sprint_speed","movement_agility","movement_reactions","movement_balance","power_shot_power","power_jumping","power_stamina","power_strength","power_long_shots","mentality_aggression","mentality_interceptions","mentality_positioning","mentality_vision","mentality_penalties","mentality_composure","defending_marking_awareness","defending_standing_tackle","defending_sliding_tackle"))
...
Apparently we kept only 43 features. Good enough. We will remove more later by performing feature selection so stay tuned.
...
**3.8.2 Individual modelling**
Now we try all models on the LW/RW pair
...
**3.9 Model merging**
Now, in order to improve accuracy, we take the best model for grouped labels with the least confusion, and add the best models for the most confused pairs using a 'Divide et impera' approach to check if we improved our accuracy.

```{r}
  final <- c()
  w<-1
  for (i in 1:nrow(test)){
    a<- as.character(predict(modellr2, test[i,], type = "class"))
    if (a == "W"){
      a <- as.character(test_predW[w])
      w <- w+1
    }
    else if (a == "CM"){a <-as.character(predict(model_ldaCM, test[i,], type = "class")$class)
    }
    final<- append(final, a)
  }

final<- as.factor(final)
accuracy(table(test_y, final))
```
...
# normalize 
players_market_norm <- as.data.frame(lapply(players_market[, c(2:41)], normalize))
...
On the other hand, [26] movement agility has a big positive impact on the market price

```{r}
colnames(players_market)
```
We check the coefficient for a generic value of lambda (50Th)
...
Morever, it is clear once again the impact of age and movement_agility on the estimation of a player market price.
...
which(lasso.coef==0)
```
Only dribbling and physic were forced to be zero 
```{r}
# residual sum of squares
...
As regard players' market price, the variables to be considered exceed the ones we were in possess of. Of course age plays a big role, since young footballers have way larger margins of improvement; but it is also true that aspects as the contract exipiry or the international profile of the player have a significant effect in real life. When the date of expiry of a player approach, the club is more inclined to agree even a lower price in order not to lose his athlete for free. Footballer players that have a huge fan base (for example on social media) will have a higher price since the club can profit from his visibility, that will result in more supporters and more revenue. 
With informations such the forementioned we are confident the predictions would be definitely more accurate.
